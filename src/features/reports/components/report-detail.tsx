...
import { parseRecommendation } from '#/features/reports/lib/parse-recommendation'
                ...
                <div key={vuln.id} className="divide-border border-border bg-card divide-y border">
                  ...
                  <div className="grid grid-cols-1 gap-4 p-4 text-xs leading-relaxed md:grid-cols-2">
                    ...
                    <div className="space-y-3">
                      ...
                      {vuln.recommendation
                        ? (() => {
                            const rec = parseRecommendation(vuln.recommendation)
                            const recSnippetId = `${vuln.id}-rec`
                            return (
                              <div>
                                <span className="text-muted-foreground mb-0.5 block font-medium">
                                  Recommendation
                                ...
                                {rec.codeExample ? (
                                  <div className="border-border bg-muted mt-2 border font-mono">
                                    <div className="border-border/40 text-muted-foreground flex items-center justify-between gap-2 border-b px-3 py-1.5 text-[10px]">
                                      <span className="flex items-center gap-1.5">
                                        Secure example
                                        {rec.codeExample.lang ? (
                                          <Badge variant="outline" className="text-[9px]">
                                            {rec.codeExample.lang}
                                          </Badge>
                                        ) : null}
                                      ...
                                      <Button
                                        ...
                                        className="hover:bg-background h-5 w-5 shrink-0"
                                        onClick={() =>
                                          handleCopySnippet(rec.codeExample!.code, recSnippetId)
                                        }
                                      >
                                        {copiedSnippetId === recSnippetId ? (
                                          <Check className="size-3 text-green-500" />
                                        ) : (
                                          <Copy className="size-3" />
                                        )}
                                    ...
                                    <pre className="overflow-x-auto p-3 text-[11px] leading-relaxed whitespace-pre">
                                      {rec.codeExample.code}
                                    </pre>
                                  </div>
                                ) : null}
                                {rec.aiDisclaimer ? (
                                  <p className="text-muted-foreground mt-1 text-[10px] italic">
                                    Example generated by AI — review and adapt to your codebase
                                    before applying.
                                  </p>
                                ) : null}
                              </div>
